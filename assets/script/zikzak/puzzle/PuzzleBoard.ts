...
export class PuzzleBoard extends cc.Component {
    ...
    getDirectionOfPiece(p0,p1){
        if(p0.curCol == p1.curCol){
            if(p0.curRow - p1.curRow > 0){
                return 0;
            }else{
                return 2;
            }
        }else{
            if(p0.curCol - p1.curCol > 0){
                return 1;
            }else{
                return 3;
            }
    ...
    public movePiece(direct,x, y): boolean {
        let piece = this.pieceMap[x][y];
        let nearPieces = this.getNearPieces(piece);
        for (let nearPiece of nearPieces) {
            console.log("xxx : ",this.getDirectionOfPiece(piece,nearPiece));
            if (this.getDirectionOfPiece(piece,nearPiece) == direct) {
